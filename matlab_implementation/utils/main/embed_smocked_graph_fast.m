function [X_res] = embed_smocked_graph_fast(SG, para, num_constr)
...
[i1, i2] = ind2sub(size(D), ...
    intersect(find(D > 0), find(D < quantile(D(:), num_constr/(size(D,1)*size(D,2)))))   );
dist_constraints = [i1, i2];
